...
function NewProjectForm() {
  ...
  // Pre-filled from client page
  const prefillHomeowner = searchParams.get('homeowner') || ''
  const prefillAddress   = searchParams.get('address')   || ''
  const fromClient       = !!prefillHomeowner

  const [form, setForm] = useState({
    address:   prefillAddress,
    name:      '',
    homeowner: prefillHomeowner,
    phone:     '',
    email:     '',
  })
  ...
  // Auto-geocode pre-filled address
  useEffect(() => {
    if (prefillAddress) geocode(prefillAddress)
  }, [])
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100dvh' }}>
      <header style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 20px', height: 52, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {fromClient && (
          <span style={{ fontSize: 12, color: 'var(--muted)', marginLeft: 4 }}>· {prefillHomeowner}</span>
        )}
      </header>
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: 480, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <Section title="Project">
          <Field label="Project name *">
            <input style={inputSt} placeholder="Front yard · Spring 2026" value={form.name}
              onChange={e => setField('name', e.target.value)} required />
          </Field>
        </Section>

        {/* Address + homeowner — only shown when creating from scratch */}
        {!fromClient && (
          <>
            ...
            <Section title="Homeowner">
              <Field label="Full name *">
                <input style={inputSt} placeholder="Jane Smith" value={form.homeowner}
                  onChange={e => setField('homeowner', e.target.value)} required />
              </Field>
