  ...
  return (
    <div style={{ maxWidth: 900, animation: 'fadeUp .3s ease both' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        <div>
          ...
          <p style={{ margin:'3px 0 0', fontSize:11, color:'rgba(255,255,255,0.22)', letterSpacing:'-0.01em' }}>
            {filteredClientEntries.length}{filteredClientEntries.length !== clientCount ? ` of ${clientCount}` : ''} client{clientCount!==1?'s':''} · {projects.length} project{projects.length!==1?'s':''} · {installedCount} installed
          </p>
        </div>
      ...
      <div style={{ position:'relative', marginBottom:10 }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="2.5" style={{ position:'absolute', left:11, top:'50%', transform:'translateY(-50%)', pointerEvents:'none' }}>
        ...
        <input
          ...
          style={{ width:'100%', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:9, color:'rgba(255,255,255,0.82)', fontSize:13, padding:'8px 32px 8px 32px', outline:'none', boxSizing:'border-box' }}
        />
        {search && (
      ...
      {!loading && (
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {filteredClientEntries.map(([key, client], ci) => {
